...
from sig.metrics import picking_evaluation


class PickingEvaluator(object_detection_evaluation.DetectionEvaluator):
    """Class to evaluate picking metrics."""
    ...
    def __init__(self,
        ...
        """
        super(PickingEvaluator, self).__init__(categories)

        self._categories = categories
        self._category_map = {category['id']: category['name']
                              for category in self._categories}

        if category_weights is None:
            category_weights = {category['id']: 1
                                for category in self._categories}
    ...
    def evaluate(self):
        ...
        for image_id in self._image_ids:
            ...
            for label_index in range(self._groundtruth_nums[image_index]):
                ...
                groundtruth.append({
                    "class_id": class_id,
                    "class_text": self._category_map[class_id],
                    "class_text_original": self._category_map[class_id],
        ...
        metrics, _ = picking_evaluation.calculate(evaluation_data,
                                                  self._category_weights,
                                                  self._score_threshold,
                                                  self._iou_threshold)

        metrics = {'PickingPrecision/' + key: value
